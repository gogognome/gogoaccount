...
class InvoicePanel extends JPanel implements Closeable {
    ...
    private JPanel createTitleBar() {
        ...
        if (mustPayingPartyBeShown()) {
            sb.append(' ');
            sb.append(textResource.getString("InvoicesSinglePartyView.paidBy", payingParty.getId() + ' ' + payingParty.getName()));
        }
    ...
    private boolean mustPayingPartyBeShown() {
        return !isInvoicePaid()
            && !invoice.getConcerningPartyId().equals(invoice.getPayingPartyId())
            && invoice.getPayingPartyId() != null;
    }
    ...
    private void addDescriptionLines() {
        for (int i=0; i<amounts.size(); i++) {
            if (amounts.get(i) != null) {
                ...
                if (amounts.get(i).isPositive()) {
                    totalDebet = totalDebet.add(amounts.get(i));
                } else {
                    totalCredit = totalCredit.subtract(amounts.get(i));
                }
            }
        }
    }
